...
export default async function AdminUpgradeRequestsPage() {
  await connectDb();
  const requests = await ListingUpgradeRequest.find().sort({ createdAt: -1 }).lean();
  const listingIds = [...new Set(requests.map((r) => String(r.listingId)))];
  ...
  return (
    <div className="space-y-4">
      <header>
        <h2 className="text-lg font-semibold text-emerald-50">Upgrade requests</h2>
        <p className="mt-1 text-sm text-white/65">Service upgrades requested from the seller dashboard.</p>
      </header>
      <div className="overflow-x-auto rounded-2xl border border-white/15 bg-black/30">
        <table className="min-w-full text-left text-sm text-white/90">
          ...
          <tbody>
            {requests.map((req) => {
              const listing = listingById.get(String(req.listingId));
              const owner = listing ? userById.get(String(listing.userId)) : undefined;
              return (
                <tr key={String(req._id)} className="border-t border-white/10 align-top">
                  <td className="px-3 py-2 whitespace-nowrap text-xs">{formatAdminDate(req.createdAt)}</td>
                  <td className="px-3 py-2">
                    <p className="font-medium">{req.upgradeName}</p>
                    <p className="text-xs text-white/50">{req.slug}</p>
                  </td>
